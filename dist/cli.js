...
export async function main(argv = process.argv.slice(2), cwd = process.cwd()) {
    try {
        ...
        if (parsed.command === "init") {
            if (!parsed.init?.writeConfig)
                throw new Error("init currently requires --write-config");
            const path = writeDefaultConfig(parsed.init.cwd, parsed.init.force);
            process.stdout.write(`Wrote ${path}\n`);
            return 0;
        }
        if (!parsed.scan)
            throw new Error("scan options were not parsed");
        const result = scan(parsed.scan);
        const rendered = renderReport(parsed.scan.format, result);
        if (parsed.scan.output)
            writeFileSync(resolve(cwd, parsed.scan.output), rendered, "utf8");
        else
